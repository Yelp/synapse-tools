...
def stat_cmd(args: argparse.Namespace,) -> int:
    return stat(INTERFACE_NAME)


def check_setup_cmd(args: argparse.Namespace,) -> int:
...
def needs_setup_cmd(args: argparse.Namespace,) -> int:
...
def clear_cmd(args: argparse.Namespace,) -> int:
...
def parse_options() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        epilog=("Setup QoS queueing disciplines for haproxy")
    )
    ...
    setup_parser = subparsers.add_parser("setup", help="Setup the qdisc")
    setup_parser.set_defaults(func=setup_cmd)

    clear_parser = subparsers.add_parser("clear", help="Clear the qdisc and iptables")
    clear_parser.set_defaults(func=clear_cmd)

    plug_parser = subparsers.add_parser("manage_plug", help="Manage the plug lane")
    ...
    protect_parser.add_argument("args", nargs=argparse.REMAINDER)
