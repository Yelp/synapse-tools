...
def check_setup(interface_name: str,) -> int:
    ...
    if tc_run[0] != 0 or iptables_run[0] != 0:
        log.info(f"No existing setup for {interface_name}")
        return 1
    ...
    if tc_result == 5 and iptables_result > 0:
        log.info(f"Expected setup exists for {interface_name}")
    else:
        log.error(f"An unexpected setup exists for {interface_name}")
        return 2
    return 0


def needs_setup(interface_name: str,) -> int:
...
def _apply_tc_rules(interface_name: str,) -> None:
    log.info(f"Creating prio qdisc with a plug lane for {interface_name}")
    tc[
        "qdisc",
        "add",
        "dev",
...
def _apply_iptables_rule(source_ip: str,) -> None:
    log.info(f"Creating iptables rule to mark outgoing syns on {source_ip}")
    iptables[
        "-t",
        "mangle",
        "-I",
...
def setup(interface_name: str, source_ip: str,) -> int:
...
def clear(interface_name: str, source_ip: str,) -> int:
    try:
...
def _manage_plug_via_netlink(interface_name: str, action_name: str = "unplug",) -> None:
    ...
    try:
        ...
        if nle.code == 22:
            ...
            log.warn(
                "Detected a non plug qdisc, likely due to an old kernel. "
    ...
    if not (
        len(nlm_response) > 0
        and nlm_response[0]["event"] == "NLMSG_ERROR"
        and nlm_response[0]["header"]["error"] is None
    ):
        raise RuntimeError(
            f"Had an error while communicating with netlink: {nlm_response}"
        )


def manage_plug(interface: str, enable_plug: bool,) -> int:
