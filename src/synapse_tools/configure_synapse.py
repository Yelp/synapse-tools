...
HAProxyTopLevelConfigExtraSections = TypedDict(
    ...
    {"listen stats": Iterable[str], "listen map.debug": Iterable[str],},
    total=False,
)
...
def get_config(synapse_tools_config_path: str,) -> SynapseToolsConfig:
    with open(synapse_tools_config_path) as synapse_config:
        return set_defaults(json.load(synapse_config))


def set_defaults(config: SynapseToolsConfig,) -> SynapseToolsConfig:
    ...
    defaults = [
        ...
        (
            ...
            "X-B3-SpanId,X-B3-TraceId,X-B3-ParentSpanId,X-B3-Flags:10,X-B3-Sampled:10",
        ),
        ...
        (
            ...
            """touch {haproxy_pid_file_path} && PID=$(cat {haproxy_pid_file_path}) && {haproxy_path} -f {haproxy_config_path} -p {haproxy_pid_file_path} -sf $PID""",
        ),
        (
        ...
        (
            "lua_dir",
            os.path.join(os.path.dirname(synapse_tools.__file__), "lua_scripts"),
        ),
        ...
        ("nginx_reload_cmd_fmt", "{nginx_reload_script} {nginx_pid_file_path}"),
        (
            "nginx_start_cmd_fmt",
            "mkdir -p {nginx_prefix} && (kill -0 $(cat {nginx_pid_file_path}) || "
            "{nginx_path} -c {nginx_config_path} -p {nginx_prefix})",
        ),
        ("nginx_check_cmd_fmt", "{nginx_path} -t -c {nginx_config_path}"),
...
def get_zookeeper_topology(zookeeper_topology_path: str,) -> Iterable[str]:
...
def _generate_nginx_top_level(
    ...
    return {
        "contexts": {
            "main": [
                "worker_processes 1",
                "worker_rlimit_nofile {}".format(
                    int(synapse_tools_config["maximum_connections"]) * 4
                ),
                "pid {}".format(synapse_tools_config["nginx_pid_file_path"]),
                "error_log {} {}".format(
...
def _generate_server_order_seed() -> int:
    return int((hashlib.md5(socket.gethostname().encode("utf-8"))).hexdigest(), 16)
...
def _generate_haproxy_top_level(
    ...
    top_level: HAProxyTopLevelConfig = {
        ...
        "reload_command": synapse_tools_config["haproxy_reload_cmd_fmt"].format(
            **synapse_tools_config
        ),
        ...
        "global": [
            ...
            "stats socket {} level admin".format(
    ...
    top_level["global"].append("setenv map_file %s" % map_file,)
    map_refresh_interval = synapse_tools_config["map_refresh_interval"]
    top_level["global"].append("setenv map_refresh_interval %d" % map_refresh_interval,)
...
def generate_base_config(
    ...
    if registration_writer == "synapse":
        base_config["file_output"] = {
            "output_directory": synapse_tools_config["file_output_path"]
        }
...
def get_backend_name(
    service_name: str, discover_type: str, advertise_type: str, endpoint_name: str,
) -> str:
...
def _get_socket_path(
    ...
    if proxy_proto:
        socket_fmt_option = synapse_tools_config[
            "haproxy_service_proxy_sockets_path_fmt"
        ]
    else:
        socket_fmt_option = synapse_tools_config["haproxy_service_sockets_path_fmt"]

    socket_path = socket_fmt_option.format(service_name=service_name)
    return socket_path


def _get_backends_for_service(
    advertise_types: Iterable[str], endpoint_timeouts: Dict[str, int],
...
def generate_acls_for_service(
    ...
    for (advertise_type, endpoint_name) in _get_backends_for_service(
        advertise_types, endpoint_timeouts,
    ):
...
def generate_configuration(
    ...
    location_depth_mapping = {
        loc: depth for depth, loc in enumerate(available_locations)
    }
    ...
    for (service_name, service_info) in services:
        ...
        namespace_is_envoy_only = (
            envoy_migration_config["migration_enabled"]
            and envoy_migration_config["namespaces"]
            .get(service_name, {"state": "synapse"})
            .get("state")
            == "envoy"
        )
        ...
        for (advertise_type, endpoint_name) in _get_backends_for_service(
            ...
            if endpoint_name != HAPROXY_DEFAULT_SECTION:
                ...
                timeout_index_list = [
                    i
                    for i, v in enumerate(config["haproxy"]["backend"])
                    if v.startswith("timeout server ")
                ]
                if len(timeout_index_list) > 0:
                    timeout_index = timeout_index_list[0]
                    config["haproxy"]["backend"][timeout_index] = (
                        "timeout server %dms" % endpoint_timeout
                    )
                else:
                    config["haproxy"]["backend"].append(
                        "timeout server %dms" % endpoint_timeout
                    )
            ...
            if proxy_port is None:
                ...
                if (
                    ...
                    and endpoint_name == HAPROXY_DEFAULT_SECTION
                ):
                    ...
                    if synapse_tools_config["listen_with_haproxy"]:
                        ...
                        config["haproxy"]["frontend"].extend(
                            [
                                f"bind {socket_path}",
                                f"bind {socket_proxy_path} accept-proxy",
                            ]
                        )
        ...
        if proxy_port is not None:
            ...
            if not namespace_is_envoy_only:
                ...
                if synapse_tools_config["listen_with_nginx"]:
                    listener_name = f"{service_name}.nginx_listener"
                    synapse_config["services"][
                        listener_name
...
def _generate_captured_request_headers(
    ...
    header_pairs = [
        pair.strip().partition(":")
        for pair in synapse_tools_config["haproxy_captured_req_headers"].split(",")
    ]
    headers = [
        "capture request header {} len {}".format(pair[0], pair[2] or "64")
        for pair in header_pairs
    ]
    return headers


def _get_default_timeout(service_info: ServiceInfo,) -> Optional[int]:
    ...
    return max(timeout_client_ms or 0, timeout_server_ms or 0)
...
def _generate_haproxy_for_watcher(
    ...
    if mode == "http":
        frontend_options.extend(
            _generate_captured_request_headers(synapse_tools_config)
        )
    ...
    if keepalive and mode == "http":
        backend_options.extend(["no option forceclose", "option http-keep-alive"])
    ...
    for header, value in extra_headers.items():
        backend_options.append(f"reqadd {header}:\ {value}")
    ...
    if len(extra_healthcheck_headers) > 0:
        ...
        headers_string = healthcheck_base + "".join(
            fr"\r\n{k}:\ {v}" for (k, v) in extra_healthcheck_headers.items()
        )
    else:
        headers_string = ""
...
def _generate_nginx_for_watcher(
    ...
    server = [f"proxy_timeout {timeout}s"]
    ...
    # on Linux < 4.4
    both_listen = (
        synapse_tools_config["listen_with_haproxy"]
        and synapse_tools_config["listen_with_nginx"]
    )
    reuseport_enabled = (
        envoy_migration_config["migration_enabled"]
        and envoy_migration_config["reuseport_enabled"]
    )
    if both_listen or reuseport_enabled:
        nginx_config["listen_options"] = "reuseport"

    service: ServiceConfig = {
        "default_servers": [{"host": "unix", "port": socket_path}],
...
def chaos_options(
    ...
    if delay:
        return (
            [
                f"tcp-request inspect-delay {delay}",
...
def get_my_grouping(grouping_type: str) -> str:
    with open(f"/nail/etc/{grouping_type}") as fd:
...
def main() -> None:
    ...
    soa_dir = os.environ.get("SOA_DIR", DEFAULT_SOA_DIR,)
    ...
    with tempfile.NamedTemporaryFile() as tmp_file:
        ...
        with open(new_synapse_config_path, "w") as fp:
            json.dump(
                new_synapse_config, fp, sort_keys=True, indent=4, separators=(",", ": ")
            )
        ...
        should_restart = not filecmp.cmp(
            new_synapse_config_path, my_config["config_file"]
        )
