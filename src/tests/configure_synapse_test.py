...
def mock_get_current_location():
    def f(typ):
        return {"region": "my_region", "superregion": "my_superregion",}[typ]

    with mock.patch(
        "synapse_tools.configure_synapse.get_current_location", side_effect=f
    ):
        yield
...
def test_get_zookeeper_topology():
    ...
    with mock.patch("synapse_tools.configure_synapse.open", m, create=True), mock.patch(
...
def test_generate_configuration(
    mock_get_current_location, mock_available_location_types
):
        ...
        services=[
            (
                "test_service",
                {
                    ...
                    "extra_headers": {"X-Mode": "ro"},
                    "extra_healthcheck_headers": {"X-Mode": "ro"},
                    "balance": "roundrobin",
    ...
    expected_configuration["services"] = {
        "test_service": {
            ...
            "discovery": {
                ...
                "label_filters": [
                    {"label": "region:my_region", "value": "", "condition": "equals",},
                ],
            },
            "haproxy": {
                "listen": [],
...
def test_generate_configuration_with_errorfiles(
    mock_get_current_location, mock_available_location_types
):
    ...
    assert (
        ...
        in actual_configuration["haproxy"]["defaults"]
    )
    assert (
...
def test_generate_configuration_single_advertise_per_endpoint_timeouts(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
...
def test_generate_configuration_single_advertise_per_endpoint_timeouts_with_default_timeout(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
...
def test_generate_configuration_single_advertise(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
...
def test_generate_configuration_with_proxied_through(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
...
def test_generate_configuration_with_nginx(
    mock_get_current_location, mock_available_location_types
):
    synapse_tools_config = configure_synapse.set_defaults(
        {"bind_addr": "0.0.0.0", "listen_with_nginx": True}
    )
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=synapse_tools_config,
        zookeeper_topology=["1.2.3.4", "2.3.4.5"],
    ...
    expected_configuration["services"] = {
        ...
        "test_service.nginx_listener": {
            "default_servers": [
                {"host": "unix", "port": "/var/run/synapse/sockets/test_service.sock",}
            ],
...
def test_generate_configuration_only_nginx(
    mock_get_current_location, mock_available_location_types
):
    synapse_tools_config = configure_synapse.set_defaults(
        {
    ...
    expected_configuration["services"] = {
        ...
        "test_service.nginx_listener": {
            ...
            "haproxy": {"disabled": True},
            "file_output": {"disabled": True},
            "nginx": {"mode": "tcp", "port": 1234, "server": ["proxy_timeout 3610s",]},
            "use_previous_backends": True,
        },
    }
...
def test_generate_configuration_with_source_required_plugin(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
...
def test_generate_configuration_with_logging_plugin(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
        ...
        services=[
            ...
            (
                "proxy_service",
                {
                    ...
                    "discover": "region",
                    "is_proxy": True,
                    "plugins": {"logging": {"enabled": True, "sample_rate": 0.25}},
                },
            ),
        ],
        envoy_migration_config=STATUS_QUO_ENVOY_MIGRATION_CONFIG,
    ...
    expected_configuration["haproxy"]["global"].extend(
        ["lua-load /nail/etc/lua_scripts/log_requests.lua", "setenv sample_rate 0.25"]
    )
...
def test_generate_configuration_with_multiple_plugins(
    mock_get_current_location, mock_available_location_types
):
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=configure_synapse.set_defaults({"bind_addr": "0.0.0.0"}),
...
def test_generate_configuration_envoy_migration_enabled(
    mock_get_current_location, mock_available_location_types
):
    ...
    config = configure_synapse.generate_configuration(
        ...
        services=[
            (
                "service_1",
                {"proxy_port": 1111, "advertise": ["region"], "discover": "region",},
            ),
            (
                "service_2",
                {"proxy_port": 2222, "advertise": ["region"], "discover": "region",},
            ),
            (
                "service_3",
                {"proxy_port": 3333, "advertise": ["region"], "discover": "region",},
            ),
            (
                "service_4",
                {"proxy_port": 4444, "advertise": ["region"], "discover": "region",},
            ),
        ],
    ...
    assert (
        config["services"]["service_1.nginx_listener"]["nginx"]["listen_options"]
        == "reuseport"
    )
    ...
    assert (
        config["services"]["service_2.nginx_listener"]["nginx"]["listen_options"]
        == "reuseport"
    )
    ...
    assert (
        config["services"]["service_3.nginx_listener"]["nginx"]["listen_options"]
        == "reuseport"
    )
    ...
    assert (
        config["services"]["service_4"]["haproxy"]["bind_address"]
        == "/var/run/synapse/sockets/service_4.sock"
    )


def test_generate_configuration_envoy_migration_disabled(
    mock_get_current_location, mock_available_location_types
):
    ...
    config = configure_synapse.generate_configuration(
        ...
        envoy_migration_config={
            "migration_enabled": False,
            "reuseport_enabled": False,
            "namespaces": {"my_service": {"state": "envoy"},},
        },
    )
...
def test_reuseport_enabled_disabled(
    mock_get_current_location, mock_available_location_types
):
    ...
    assert config["services"]["service_1.nginx_listener"]["nginx"]["port"] == 1111
    assert (
        "listen_options" not in config["services"]["service_1.nginx_listener"]["nginx"]
    )


def test_reuseport_enabled_and_migration_enabled_disabled(
    mock_get_current_location, mock_available_location_types
):
...
def test_registration_writer_not_in_migration_config(
    mock_get_current_location, mock_available_location_types
):
...
def test_registration_writer_set_to_synapse_in_migration_config(
    mock_get_current_location, mock_available_location_types
):
...
def test_registration_writer_not_synapse_in_migration_config(
    mock_get_current_location, mock_available_location_types
):
...
def setup_mocks_for_main(tmpdir, config_file_path):
    ...
    synapse_tools_config.write(json.dumps({"config_file": config_file_path,}))
...
def test_synapse_restarted_when_config_files_differ(tmpdir):
    ...
    with setup_mocks_for_main(tmpdir, config_path.strpath,) as (
        mock_subprocess_check_call,
        mock_generate_configuration,
    ):
...
def test_chaos_error_503(mock_get_current_location, mock_available_location_types):
    with mock.patch.object(configure_synapse, "get_my_grouping") as grouping_mock:
        ...
        assert (
            actual_configuration["services"]["test_service"]["discovery"]["method"]
            == "base"
        )


def test_discovery_only_services(
    mock_get_current_location, mock_available_location_types
):
    synapse_tools_config = configure_synapse.set_defaults(
        {
...
def test_nginx_no_proxy_proto(mock_get_current_location, mock_available_location_types):
    synapse_tools_config = configure_synapse.set_defaults(
        {"listen_with_nginx": True, "nginx_proxy_proto": False,}
    )
    actual_configuration = configure_synapse.generate_configuration(
        synapse_tools_config=synapse_tools_config,
        zookeeper_topology=["1.2.3.4", "2.3.4.5"],
        services=[("test_service", {"proxy_port": 1234}),],
        envoy_migration_config=STATUS_QUO_ENVOY_MIGRATION_CONFIG,
    )
    ...
    assert (
        nginx["default_servers"][0]["port"]
        == "/var/run/synapse/sockets/test_service.sock"
    )
...
def test_nginx_proxy_proto(mock_get_current_location, mock_available_location_types):
    ...
    assert (
        nginx["default_servers"][0]["port"]
        == "/var/run/synapse/sockets/test_service.prxy"
    )
