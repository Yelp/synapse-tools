...
SERVICES = {
    ...
    "service_three_endpoint_timeout.main": {
        ...
        "ip_address": socket.gethostbyname(
            CONTAINER_PREFIX + "servicethree_endpoint_timeout_1"
        ),
        "port": 1024,
        "proxy_port": 20070,
        "mode": "http",
        ...
        "discover": "habitat",
        "advertise": ["habitat", "region"],
        "endpoint_timeouts": {"/example/endpoint": 1000,},
    },
    # HTTP service with a custom endpoint
...
YIELD_PARAMS = [
    item for sublist in SYNAPSE_TOOLS_CONFIGURATIONS.values() for item in sublist
]
...
def setup(request):
    ...
    try:
        ...
        for name, data in SERVICES.iteritems():
            labels = {
                f"{advertise_typ}:my_{advertise_typ}": ""
        ...
        subprocess.check_call(
            ["configure_synapse"],
            env=dict(os.environ, SYNAPSE_TOOLS_CONFIG_PATH=request.param),
        )
...
class TestGroupOne:
    def test_haproxy_config_valid(self, setup):
        subprocess.check_call(
            ["haproxy-synapse", "-c", "-f", "/var/run/synapse/haproxy.cfg"]
        )
    ...
    def test_backup_http_synapse_service_config(self, setup):
        ...
        actual_service_entry = synapse_config["services"].get(
            "service_three.main.region"
        )
    ...
    def test_hacheck(self, setup):
        for name, data in SERVICES.iteritems():
            ...
            url = "http://{}:6666/http/{}/0{}".format(
                data["ip_address"], name, data["healthcheck_uri"]
            )

            headers = {
                "X-Haproxy-Server-State": "UP 2/3; host=srv2; port=%d; name=bck/srv2;"
            ...
            with contextlib.closing(
                urllib2.urlopen(request, timeout=SOCKET_TIMEOUT)
            ) as page:
    ...
    def test_synapse_haproxy_stats_page(self, setup):
        ...
        with contextlib.closing(
            urllib2.urlopen(haproxy_stats_uri, timeout=SOCKET_TIMEOUT)
        ) as haproxy_stats:
            reader = csv.DictReader(haproxy_stats)
            rows = [
                (row["# pxname"], row["svname"], row["check_status"]) for row in reader
            ]
    ...
    def test_http_service_is_accessible_using_haproxy(self, setup):
        for name, data in SERVICES.iteritems():
            if data["mode"] == "http" and "chaos" not in data:
                uri = "http://localhost:%d%s" % (
                    data["proxy_port"],
                    data["healthcheck_uri"],
                )
                with contextlib.closing(
                    urllib2.urlopen(uri, timeout=SOCKET_TIMEOUT)
                ) as page:
                    assert page.read().strip() == "OK"
        ...
        for name, data in SERVICES.iteritems():
            if data["mode"] == "tcp":
                s = socket.create_connection(
                    address=(data["ip_address"], data["port"]), timeout=SOCKET_TIMEOUT
                )
                s.close()
    ...
    def test_source_required_plugin(self, setup):
        ...
        if "nginx" not in setup and "both" not in setup:
            ...
            request = urllib2.Request(
                url=url, headers={"X-Smartstack-Origin": "Spoof-Value"}
            )
            with contextlib.closing(
                urllib2.urlopen(request, timeout=SOCKET_TIMEOUT)
            ) as page:
